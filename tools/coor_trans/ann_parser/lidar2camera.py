...
from .hw_pb.hardware_config_pb2 import HardwareConfig, CameraParameter, LidarParameter
import  math
...
def getCalibTransformMatrix(json_file, camera_name, lidar_name="FRONT_LIDAR",cali_file=False):
    if not cali_file:
    ...
    return Tlv_params, Tvc_params, Tcv_params
...
def vehicle2lidar(position: list, lv_trans: tuple):
    vehicle_position = np.array(position).reshape(3, 1)
    # lidar2vehicle
    lv_R, lv_T = lv_trans
    lidar_position = np.matmul(np.linalg.inv(lv_R),  np.array(vehicle_position).reshape(3, 1) - lv_T)

    return list(lidar_position.reshape(-1))

def vehicle2lidar_2(position: list, yaw: float, lv_trans: tuple):
    position = np.array(position).reshape(3, 1)
    vehicle_yaw_point = [position[0]+math.cos(yaw)*1, position[1]+math.sin(yaw)*1, position[2]]
    lv_R, lv_T = lv_trans

    lidar_position = np.matmul(np.linalg.inv(lv_R),  np.array(np.array(position).reshape(3, 1)).reshape(3, 1) - lv_T)
    lidar_yaw_point = np.matmul(np.linalg.inv(lv_R),  np.array(np.array(vehicle_yaw_point).reshape(3, 1)).reshape(3, 1) - lv_T)

    lidar_yaw = math.atan2(lidar_yaw_point[1]-lidar_position[1],lidar_yaw_point[0]-lidar_position[0])
    return list(lidar_position.reshape(-1)), lidar_yaw

def lidar2vehicle_2(position: list, yaw: float, lv_trans: tuple):
    position = np.array(position).reshape(3, 1)
    vehicle_yaw_point = [position[0]+math.cos(yaw)*1, position[1]+math.sin(yaw)*1, position[2]]
    lv_R, lv_T = lv_trans

    vehicle_position =  np.matmul(lv_R, np.array(position).reshape(3, 1)) + lv_T
    vehicle_yaw_point = np.matmul(lv_R, np.array(position).reshape(3, 1)) + lv_T

    vehicle_yaw = math.atan2(vehicle_yaw_point[1]-vehicle_position[1],vehicle_yaw_point[0]-vehicle_position[0])
    return list(vehicle_position.reshape(-1)), vehicle_yaw
