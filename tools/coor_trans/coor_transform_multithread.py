...
def parse_args():
    ...
    parser.add_argument("-i", "--input_txt_path", type=str,
                        default="/data_path/sample_train_data_side.txt", help="input data path")
    parser.add_argument("-o", "--output_dir", type=str,
                        default="/root/coor_transform_output_sampler", help="output dir")
...
def main():
    ...
    def worker(_):
        ...
        idx = json_path.split('/')[-3] + json_path.split('/')[-2] + json_path.split('/')[-1]
        save_new_json_path = os.path.join(opt.output_dir, idx)
        dataset.write_json(save_new_json_path, json_map)

    with ThreadPool(processes = 40) as pool:
        list(tqdm(pool.imap(worker, lines), total=len(lines), desc='DataFrame Loading'))
        pool.terminate()


    # for idx, line in tqdm(enumerate(combined_lst), total=len(combined_lst), desc="Coordinate transforming"):
    #     # json_path = line.strip()
    #     # json_map = dataset.get_ann_info(json_path)
    #     # print(json_map)
    #     save_new_json_path = os.path.join(opt.output_dir, str(idx)+'.json')
    #     dataset.write_json(save_new_json_path, line)
